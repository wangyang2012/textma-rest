...
public class TjLiaisonClientRestSource {
    ...
    // http://localhost:9090/textma/tjLiaisonClients?search=
    @GetMapping(value = "/tjLiaisonClients")
    public WebixDatatableResponse<TeClient> listTjLiaisonClients(@RequestParam(defaultValue = "", required = false) String search, @RequestParam(defaultValue = "20", required = false) Integer count, @RequestParam(defaultValue = "0", required = false) Integer start, @RequestParam(required = false) Map<String, String> filter) {
        Integer page = start / count;

        Sort sort = getSortInfo(filter);

        Pageable pageable = new PageRequest(page, count, sort);
        Page<TeClient> clientMeres = tjClientLiaisonService.findAllClientMere(search, pageable);
        return new WebixDatatableResponse<TeClient>(clientMeres, start);
    }
